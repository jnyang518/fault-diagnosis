...
# [修改点 1] 删除了 ConvEmbedding 类，直接在模型中使用 Linear
...
class SVD_Transformer_Shared(nn.Module):  # 重命名：去掉了CNN前缀
    def __init__(self, input_dim, num_classes, d_model, nhead, num_layers, rank, dropout, noise_std):
        super(SVD_Transformer_Shared, self).__init__()

        # [修改点 2] 替换为线性映射层
        # 将输入特征维度 (1) 映射到 模型维度 (d_model)
    ...
    def forward(self, src):
        # src shape: [Batch, Seq_Len, Feature_Dim(1)]

        # 1. 线性嵌入
        src = self.embedding(src)  # -> [Batch, Seq_Len, d_model]

        # 2. 调整维度以适应 PositionalEncoding (它期望 [Seq_Len, Batch, d_model])
        src = src.permute(1, 0, 2)
        src = self.pos_encoder(src)
        src = src.permute(1, 0, 2)  # 变回 [Batch, Seq_Len, d_model]
