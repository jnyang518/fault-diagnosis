...
# ================= 1. 全局配置参数 =================
USE_DUMMY_DATA = False  # 【注意】如果没有真实csv文件，请保持为True
...
# --- 模块 A: 卷积嵌入层 (Convolutional Embedding) ---
class ConvEmbedding(nn.Module):
    """
    使用 CNN 提取局部特征，替代简单的 Linear 投影
    ...
    def forward(self, x):
        ...
        # 转回 (Batch, Seq_Len, d_model) 供 Transformer 使用
        x = x.permute(0, 2, 1)
        return x
...
class SVDTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, rank, dropout=0.1):
        ...
        # FFN 部分保持标准 Linear，主要为了展示 Attention 的 SVD 效果
        # 如果追求极致压缩，这里也可以换成 SVDLinear
...
class CNN_SVD_Transformer(nn.Module):
    def __init__(self, input_dim, num_classes, d_model, nhead, num_layers, rank, dropout):
        ...
        # 【改进】使用 ConvEmbedding 替代普通 Linear
    ...
    def forward(self, src):
        # 1. 卷积特征提取 (Batch, Seq, 1) -> (Batch, Seq, d_model)
        src = self.embedding(src)

        # 2. 变换维度适配 PosEncoder: (Seq, Batch, d_model)
        src = src.permute(1, 0, 2)
        src = self.pos_encoder(src)

        # 3. 变换维度适配 Attention: (Batch, Seq, d_model)
        src = src.permute(1, 0, 2)
...
print("\n" + "=" * 40)
print(f"模型构建完成 (CNN-SVD-Transformer)")
print(f"当前参数量: {total_params:,}")
print("结构亮点:")
print("1. [ConvEmbedding] 包含2层1D卷积，提取局部信号特征")
print(f"2. [SVD Attention] 使用 Rank={SVD_RANK} 的低秩矩阵逼近全连接")
print("=" * 40 + "\n")
...
def train_and_validate(model, train_loader, test_loader, epochs):
    ...
    for epoch in range(epochs):
        ...
        print(
            f"Epoch {epoch + 1:02d}/{epochs} | Train Loss: {avg_train_loss:.4f} | Test Loss: {avg_test_loss:.4f} | Test Acc: {acc:.2f}%")
...
def plot_history(history):
    ...
    plt.plot(history['test_loss'], label='Val Loss', color='red', linestyle='--')
    plt.title('训练与验证 Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True)
    ...
    plt.subplot(1, 2, 2)
    plt.plot(history['test_acc'], label='Val Accuracy', color='green', marker='o')
    plt.title('验证集准确率')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.grid(True)
...
def plot_confusion_matrix_and_report(model, loader, classes):
    ...
    plt.title('混淆矩阵 (CNN-SVD-Transformer)')
    plt.ylabel('真实标签')
    plt.xlabel('预测标签')
    plt.show()
